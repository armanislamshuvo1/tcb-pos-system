...
const PRESET_REASONS = [
  ...
  'Wrong customer / room ticket',
  'Need to add more items to bill',
  'Cashier error / test settlement'
];
...
export default function RevertSettlementModal({ isOpen, txn, onClose, onRevertSuccess }) {
  ...
  const handleConfirmRevert = useCallback(async () => {
    if (!reason.trim()) {
      setErrorMsg('Please specify or select a reason for reverting');
      return;
    }
    ...
    try {
      ...
      setErrorMsg(err.response?.data?.message || 'Failed to revert settlement. Check PIN.');
    } finally {
      setSubmitting(false);
    }
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e) => {
      // Don't intercept typing if user is in textarea or input
  ...
  }, [isOpen, pinCode, reason, handleConfirmRevert, onClose]);
  ...
  const ticketLabel = txn.txnNumber || (txn.transactionIds ? `${txn.transactionIds.length} tickets` : 'Selected Ticket');
  const amountToRevert = txn.grandTotalInCents || txn.totalSettledInCents || 0;
  ...
  return (
    <div className="fixed inset-0 z-60 bg-black/85 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 overflow-y-auto animate-in fade-in duration-150">
      ...
      {/* Modal Container */}
      <div className="relative bg-slate-900 border border-amber-500/30 rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col z-10 animate-in zoom-in-95 duration-150">
        {/* Header */}
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-base sm:text-lg font-black text-white flex items-center space-x-2">
                <span>Revert Bill Settlement</span>
              </h2>
              <div className="flex items-center space-x-2 text-xs font-mono text-amber-400 font-bold">
                <span>{ticketLabel}</span>
                <span className="text-slate-500">•</span>
                <span className="text-white font-sans">{formatCurrency(amountToRevert, currency)}</span>
                {txn.paymentMethod && (
                  <>
                    <span className="text-slate-500">•</span>
                    <span className="text-emerald-400 font-sans text-[11px]">via {txn.paymentMethod}</span>
                  </>
                )}
              </div>
            </div>
        ...
        <div className="p-5 sm:p-6 space-y-5 max-h-[75vh] overflow-y-auto">
          ...
          <div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-2xl flex items-start space-x-2.5 text-xs text-amber-200">
            <AlertCircle className="w-4 h-4 text-amber-400 shrink-0 mt-0.5" />
            <div>
              <strong className="text-amber-300 font-semibold block">Reopen Bill as Unpaid Tab</strong>
              <span>
                This will revert the settlement and return the bill status to <strong className="text-blue-300">Unpaid Tab (TAB_DEFERRED)</strong>. The collected payment will be removed from settled revenue, and the bill will reappear in active tabs for settlement.
              </span>
            </div>
          </div>
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold text-slate-300 flex items-center space-x-1.5">
              ...
              <span>Reason for Reversion (Required)</span>
            </label>
            ...
            <textarea
              ...
              placeholder="Enter or customize the revert reason..."
          ...
          <div className="space-y-3 pt-2 border-t border-slate-800">
            <div className="flex items-center justify-between">
              <label className="text-xs font-bold text-slate-300 flex items-center space-x-1.5">
                ...
                <span>Authorizing Cashier PIN ({user?.fullName || 'Current Cashier'})</span>
              </label>
